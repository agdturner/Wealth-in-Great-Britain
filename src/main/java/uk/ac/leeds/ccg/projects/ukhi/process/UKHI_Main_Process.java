/*
 * Copyright 2018 Andy Turner, University of Leeds, UK.
 *
 ...
 */
package uk.ac.leeds.ccg.projects.ukhi.process;
...
import uk.ac.leeds.ccg.projects.ukhi.core.UKHI_Environment;
import uk.ac.leeds.ccg.projects.ukhi.core.UKHI_Object;
import uk.ac.leeds.ccg.data.waas.data.WaAS_Collection;
import uk.ac.leeds.ccg.data.waas.data.records.WaAS_CombinedRecord;
import uk.ac.leeds.ccg.data.waas.data.WaAS_GORSubsetsAndLookups;
import uk.ac.leeds.ccg.data.waas.data.handlers.WaAS_HHOLD_Handler;
import uk.ac.leeds.ccg.data.waas.data.handlers.WaAS_Handler;
...
import uk.ac.leeds.ccg.data.waas.data.hhold.WaAS_W2HRecord;
...
import uk.ac.leeds.ccg.data.waas.data.id.WaAS_WID;
...
import uk.ac.leeds.ccg.projects.ukhi.chart.UKHI_LineGraph;
import uk.ac.leeds.ccg.projects.ukhi.core.UKHI_Strings;
import uk.ac.leeds.ccg.projects.ukhi.io.UKHI_Files;
...
public class UKHI_Main_Process extends UKHI_Object {
    ...
    public static void main(String[] args) {
        try {
            ...
            UKHI_Environment env = new UKHI_Environment(de);
     ...
     * associated with meeting landlord obligations).How does the balance
     * between costs and benefits vary for households in different tenures and
     * property wealth percentiles, as well as different regions, income
     * percentiles and age cohorts? Which groups are the biggest gainers and
     * losers over the period 2006-2016? To what extent are the costs and
     * benefits affecting households consistent over time? In other words, how
     * much mobility do households experience between the categories of
     * ‘winners’ and ‘losers’ over the course of those ten years?
     *
    ...
    public void run() throws FileNotFoundException, IOException,
        ...
        subset = (HashSet<WaAS_W1ID>) env.we.hh.getSubset(4);
        /**
         * Init gors, GORNameLookup, GORSubsets and GORLookups.
         */
        if (true) {
            gors = WaAS_Handler.getGORs();
            GORNameLookup = (TreeMap<Byte, String>) WaAS_Handler.getGORNameLookup();
            GORSubsetsAndLookups = env.we.hh.getGORSubsetsAndLookups("", gors, subset);
        }

        int[] ttotals = new int[we.NWAVES];
        env.env.log("GOR Subsets");
        env.env.log("NW1,NW2,NW3,NW4,NW5,GORNumber,GORName");
        ...
        TreeMap<WaAS_W5ID, WaAS_W5Record> w5recs = (TreeMap<WaAS_W5ID, WaAS_W5Record>) w5.lookup;
        ...
        env.env.log("" + w5recs.size() + "\t countAllW5withW4");
        env.env.log("" + countMortgage + "\t countMortgage");
        env.env.log("" + countNonMortgage + "\t countNonMortgage");
        env.env.log("" + countBuyWithMortgage + "\t countBuyWithMortgage");
        env.env.log("" + countPartBuyWithMortgage + "\t countPartBuyWithMortgage");
        env.env.log("" + countZeroMIntRate1W5 + "\t countZeroMIntRate1W5");
        env.env.log("" + countPositiveMIntRate1W5 + "\t countPositiveMIntRate1W5");
        env.env.log("" + countZeroMVal1W5 + "\t countZeroMVal1W5");
        env.env.log("" + countPositiveMVal1W5 + "\t countPositiveMVal1W5");
        env.env.log("" + countZeroMNumbNW5 + "\t countZeroMNumbNW5");
        env.env.log("" + countPositiveMNumbNW5 + "\t countPositiveMNumbNW5");
        env.env.log("" + countGT1MNumbNW5 + "\t countGT1MNumbNW5");
...
//	Value = -6.0	Label = Error Partial
        ...
        long tDVLUKVAL;
        if (wave == we.W1) {
            tDVLUKVAL = env.we.data.getData().keySet().stream().mapToLong(x -> {
                WaAS_CollectionID cID = (WaAS_CollectionID) x;
                long cDVLUKVAL = 0;
                    WaAS_Collection c = env.we.data.getCollection(cID);
                    cDVLUKVAL = c.data.keySet().stream().mapToLong(y -> {
                        WaAS_W1ID w1ID = (WaAS_W1ID) y;
                        int DVLUKVAL = 0;
                ...
                WaAS_Collection c = env.we.data.getCollection(cID);
                 cDVLUKVAL = c.data.keySet().stream().mapToLong(y -> {
                    ...
                    if (subset.contains(w1ID)) {
                        ...
                        while (ite.hasNext()) {
                            WaAS_W2ID w2ID = ite.next();
                            HashMap<WaAS_W3ID, WaAS_W3Record> w3_2 = (HashMap<WaAS_W3ID, WaAS_W3Record>) cr.w3Recs.get(w2ID);
                 ...
                 cDVLUKVAL = c.data.keySet().stream().mapToLong(y -> {
                    ...
                    if (subset.contains(w1ID)) {
                        ...
                        while (ite.hasNext()) {
                            ...
                            while (ite2.hasNext()) {
                                WaAS_W3ID w3ID = ite2.next();
                                Map<WaAS_W4ID, Map<WaAS_W5ID, WaAS_W5Record>> w5_3;
                                w5_3 = w5_2.get(w3ID);
        ...
        env.env.log("Total (Hhold aggregate) DVLUKVAL in Wave " + wave + " " + tDVLUKVAL);
        return tDVLUKVAL;
    }
    ...
    public long getFINCVB(HashSet<WaAS_W1ID> subset, byte wave) {
        long tFINCVB;
        if (wave == we.W1) {
            tFINCVB = env.we.data.getData().keySet().stream().mapToLong(x -> {
                WaAS_CollectionID cID = (WaAS_CollectionID) x;
                long cFINCVB = 0;
                WaAS_Collection c = env.we.data.getCollection(cID);
                 cFINCVB = c.data.keySet().stream().mapToLong(y -> {
                    WaAS_W1ID w1ID = (WaAS_W1ID) y;
                    byte FINCVB = 0;
                    if (subset.contains(w1ID)) {
                        WaAS_CombinedRecord cr = (WaAS_CombinedRecord) c.data.get(w1ID);
        ...
        env.env.log("Total (Person aggregate) FINCVB in Wave " + wave + " " + tFINCVB);
        return tFINCVB;
    }
     ...
     */
    public Map<Byte, Map<WaAS_WID, Integer>> getHPRICE(
            HashSet<WaAS_W1ID> subset,
            WaAS_GORSubsetsAndLookups gORSubsetsAndLookups,
            byte wave) {
        // Initialise result
        Map<Byte, Map<WaAS_WID, Integer>> r = new HashMap<>();
        ...
        // For brevity/convenience.
        if (wave == we.W1) {
            env.we.data.getData().keySet().stream().forEach(x -> {
                WaAS_CollectionID cID = (WaAS_CollectionID) x;
                WaAS_Collection c = env.we.data.getCollection(cID);
                c.data.keySet().stream().forEach(y -> {
                    WaAS_W1ID w1ID = (WaAS_W1ID) y;
                    if (subset.contains(w1ID)) {
                        WaAS_CombinedRecord cr = (WaAS_CombinedRecord) c.data.get(w1ID);
                ...
                c.data.keySet().stream().forEach(y -> {
                    ...
                    if (subset.contains(w1ID)) {
                        ...
                        while (ite1.hasNext()) {
                            ...
                            Map<WaAS_W3ID, WaAS_W3Record> w3_2;
    ...
    public void createLineGraph(String title, String xAxisLabel,
            ...
            BigDecimal yIncrement) throws IOException {
        ...
        Path outdir = gf.getOutputDir();
        String filename;
        filename = title.replace(" ", "_");
        file = Paths.get(outdir.toString(), filename + "." + format);
